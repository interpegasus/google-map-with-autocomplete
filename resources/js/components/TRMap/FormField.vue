  ...
  >
    <template #field>
      <div class="space-y-4">
        ...
        <div class="w-1" v-if="!hidePlaceId">
          <p class="mb-1">Place ID</p>
          <input
              type="text"
              class="w-full form-control form-input form-input-bordered"
              :class="errorClasses"
...
export default {
  ...
  mounted() {
    Nova.$on('google-place-id-update', data => {
      this.placeId = data
    })
    Nova.$on('google-place-name-update', data => {
      this.name = data
    })
  ...
  methods: {
    setInitialValue() {
      ...
      if (this.field.hidePlaceId) {
        this.hidePlaceId = this.field.hidePlaceId
      }
      if (this.field.hideName) {
        this.hideName = this.field.hideName
      }
    ...
    fill(formData) {
      ...
      formData.append(this.field.attribute + '[placeId]', this.placeId)
      formData.append(this.field.attribute + '[name]', this.name)
      formData.append(
